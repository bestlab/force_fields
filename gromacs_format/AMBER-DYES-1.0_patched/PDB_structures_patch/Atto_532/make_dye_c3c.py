...
template_pdblines = list(filter(lambda x: x[0:6]=="ATOM  ", open("T51_C3C.pdb").readlines()))

linker_lines = list(filter(lambda x: x.find(' C3C ')>0, template_pdblines))
template_dye_lines = list(filter(lambda x: x.find(' T51 ')>0, template_pdblines))
...
for line in template_dye_lines:
    if line.find("C99") > 0:
        o_t = get_xyz(line)
    elif line.find("O91")>0:
        a_t = get_xyz(line)
    elif line.find("C16")>0:
        b_t = get_xyz(line)
